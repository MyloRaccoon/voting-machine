...
use crate::storages::file::{ScoreboardDao, VotingMachineDao};
...
impl From<ScoreboardDao> for Scoreboard {
    fn from(scoreboard_dao: ScoreboardDao) -> Self {
        ...
        for score in scoreboard_dao.scores {
            scores.insert(Candidate(score.0), Score(score.1));
        }
        let blank_score = Score(scoreboard_dao.blank_score);
        let invalid_score = Score(scoreboard_dao.invalid_score);
        Self { scores, blank_score, invalid_score }
    }
}
...
impl From<VoteForm> for BallotPaper {
    ...
    fn from(form: VoteForm) -> Self {
        ...
        let candidate = match form.candidate.is_empty() {
            true => None,
            false => Some(Candidate(form.candidate)),
        };
...
impl From<VotingMachineDao> for VotingMachine {
    fn from(voting_machine: VotingMachineDao) -> Self {
        let mut voters = AttendenceSheet(Set::<Voter>::new());
        for voter in voting_machine.voters {
            voters.0.insert(Voter(voter));
        }
        let scoreboard = Scoreboard::from(voting_machine.scoreboard);
        Self { voters, scoreboard }
    }
}
