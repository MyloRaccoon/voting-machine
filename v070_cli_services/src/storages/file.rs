...
mod tests {
	...
	async fn test_get_put_machine_in_file() {
		...
		let mut file = FileStore::create(voting_machine.clone(), "test_get_put_machine_in_file.json").await.expect("FileStore::new returned Err");
	...
	async fn test_double_file_instance() {
		let candidates = vec![Candidate("Godot".to_string()), Candidate("Unity".to_string())];
		let mut voting_machine = VotingMachine::new(candidates);
		let mut file = FileStore::create(voting_machine.clone(), "test_double_file_instance.json").await.expect("FileStore::new returned Err");
		let file2 = FileStore::create(voting_machine.clone(), "test_double_file_instance.json").await.expect("FileStore::new 2 returned Err");
